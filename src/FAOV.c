...
double AOV(Vector *time, Vector *magnitude, int n, int bins, double freq,
           double mean) {
  /*
  ...
  */
  ...
  for (int i = 0; i < num_of_vec; i++) {
    phase[i].simd_vector =
        _mm256_sub_pd(_mm256_mul_pd(time[i].simd_vector, freq_vec.simd_vector),
                      _mm256_floor_pd(_mm256_mul_pd(time[i].simd_vector,
                                                    freq_vec.simd_vector)));
  }
  ...
  for (int i = 0; i < num_of_vec; i++) {
    square.simd_vector =
        _mm256_mul_pd(magnitude[i].simd_vector, magnitude[i].simd_vector);
    ide.simd_vector = _mm256_floor_pd(
        _mm256_mul_pd(support.simd_vector, phase[i].simd_vector));
    for (int j = 0; j < 4; j++) {
      id = (int)ide.array[j];
...
double period(double *time, double *magnitude, int n, int bins, double t_max) {
  ...
  for (int i = 0; i < num; i++) {
    ...
    mean_vect.simd_vector =
        _mm256_add_pd(mean_vect.simd_vector, mag[i].simd_vector);
  }
  double mean = 0;
  for (int i = 0; i < 4; i++) {
...
double period_time_domain(double *time, double *magnitude, int n, int bins,
                          double t_min, double t_max, int num_points) {
  int num = n / 4;
  Vector mag[num];
  Vector t[num];
  Vector mean_vect;
