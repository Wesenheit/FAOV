...
PyObject *Power(PyObject *self, PyObject *args) {
  ...
  */
  ...
  double t_max;
  int bins;
  if (!PyArg_ParseTuple(args, "O!O!id", &PyArray_Type, &t_arr, &PyArray_Type,
                        &mag_arr, &bins, &t_max)) {
...
static PyMethodDef FAOVMethods[] = {
    {"Period", Period_py, METH_VARARGS, "Find period using AOV."},
    {"Power", Power, METH_VARARGS, "Return freq and power arrays"},
    {NULL, NULL, 0, NULL}};

static struct PyModuleDef FAOV_module = {PyModuleDef_HEAD_INIT, "FOAV", NULL,
                                         -1,

                                         FAOVMethods};

PyMODINIT_FUNC PyInit_FAOV(void)
